 ...
 * Properties Model
 */
class RealestatenowModelProperties extends JModelList
{
	...
	public function getItems()
	{
		...
		if (RealestatenowHelper::checkArray($items))
		{
			foreach ($items as $nr => &$item)
			{
				...
				$item->featured = $this->selectionTranslation($item->featured, 'featured');
			}
		}
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.name LIKE '.$search.' OR a.cityid LIKE '.$search.' OR g.name LIKE '.$search.' OR a.stateid LIKE '.$search.' OR h.name LIKE '.$search.' OR a.mls_id LIKE '.$search.' OR a.street LIKE '.$search.')');
			}
		}
		...
		// [Interpretation 9255] Filter by Featured.
		if ($featured = $this->getState('filter.featured'))
		{
			$query->where('a.featured = ' . $db->quote($db->escape($featured)));
		}
	...
	public function getExportData($pks)
	{
		...
		if (RealestatenowHelper::checkArray($pks))
		{
			...
			// [Interpretation 8859] From the realestatenow_property table
			$query->from($db->quoteName('#__realestatenow_property', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__realestatenow_property");
