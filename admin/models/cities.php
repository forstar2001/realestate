...
class RealestatenowModelCities extends JModelList
{
	...
	public function getItems()
	{
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			{
				$access = (JFactory::getUser()->authorise('city.access', 'com_realestatenow.city.' . (int) $item->id) && JFactory::getUser()->authorise('city.access', 'com_realestatenow'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->select($db->quoteName('g.name','stateid_name'));
		$query->join('LEFT', $db->quoteName('#__realestatenow_state', 'g') . ' ON (' . $db->quoteName('a.stateid') . ' = ' . $db->quoteName('g.id') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.name LIKE '.$search.' OR a.stateid LIKE '.$search.' OR g.name LIKE '.$search.')');
			}
		}
	...
	public function getExportData($pks)
	{
		...
		if (RealestatenowHelper::checkArray($pks))
		{
			...
			// [Interpretation 8859] From the realestatenow_city table
			$query->from($db->quoteName('#__realestatenow_city', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__realestatenow_city");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__realestatenow_city'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
