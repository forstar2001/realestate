...
/**
 * Realestatenow Rent_type Model
 */
class RealestatenowModelRent_type extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.rent_type';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'rent_type', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.rent_type');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.rent_type', 'rent_type', $options);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_realestatenow.rent_type.' . (int) $id))
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/rent_type.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_realestatenow.rent_type.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_realestatenow.rent_type.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_realestatenow.rent_type.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_rent_type'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.rent_type.data', array());
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= RealestatenowHelper::getActions('rent_type');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('rent_type', 'realestatenowTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_REALESTATENOW_RENT_TYPE_SAVE_WARNING');
				}
