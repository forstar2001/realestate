...
/**
 * Realestatenow Agency Model
 */
class RealestatenowModelAgency extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.agency';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'agency', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.agency');
			}
		}
		$this->listofficevvvy = $item->id;
	 ...
	 */
	public function getVwgproperty_listings()
	{
		...
		// [Interpretation 8502] Filter by listofficevvvy global.
		$listofficevvvy = $this->listofficevvvy;
		if (is_numeric($listofficevvvy ))
		{
			$query->where('a.listoffice = ' . (int) $listofficevvvy );
		}
		elseif (is_string($listofficevvvy))
		{
			$query->where('a.listoffice = ' . $db->quote($listofficevvvy));
		}
		else
		{
			$query->where('a.listoffice = -5');
		}
		...
		if ($db->getNumRows())
		{
			...
			if (RealestatenowHelper::checkArray($items))
			{
				foreach ($items as $nr => &$item)
				{
					...
					$item->featured = $this->selectionTranslationVwgproperty_listings($item->featured, 'featured');
				}
			}
	 ...
	 */
	public function selectionTranslationVwgproperty_listings($value,$name)
	{
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.agency', 'agency', $options);
		...
		if ($id != 0 && (!$user->authorise('agency.edit.state', 'com_realestatenow.agency.' . (int) $id))
			|| ($id == 0 && !$user->authorise('agency.edit.state', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('agency.edit.created_by', 'com_realestatenow.agency.' . (int) $id))
			|| ($id == 0 && !$user->authorise('agency.edit.created_by', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('agency.edit.created', 'com_realestatenow.agency.' . (int) $id))
			|| ($id == 0 && !$user->authorise('agency.edit.created', 'com_realestatenow')))
		{
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/agency.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('agency.delete', 'com_realestatenow.agency.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('agency.edit.state', 'com_realestatenow.agency.' . (int) $recordId);
		...
		return $user->authorise('agency.edit.state', 'com_realestatenow');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('agency.edit', 'com_realestatenow.agency.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('agency.edit',  'com_realestatenow');
	}
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_agency'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.agency.data', array());
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('agency.create') && !$this->canDo->get('agency.batch'))
		{
			return false;
		}
		...
		elseif (isset($values['published']) && !$this->canDo->get('agency.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('agency.edit', $contexts[$pk]))
			{
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('agency.edit') && !$this->canDo->get('agency.batch'))
		{
		...
		if (isset($values['published']) && !$this->canDo->get('agency.edit.state'))
		{
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('agency', 'realestatenowTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_REALESTATENOW_AGENCY_SAVE_WARNING');
				}
