...
/-------------------------------------------------------------------------------------------------------/
	...
	@subpackage		favorite_listing.php
...
/**
 * Realestatenow Favorite_listing Model
 */
class RealestatenowModelFavorite_listing extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.favorite_listing';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'favorite_listing', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.favorite_listing');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.favorite_listing', 'favorite_listing', $options);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_realestatenow.favorite_listing.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('favorite_listing.edit.created_by', 'com_realestatenow.favorite_listing.' . (int) $id))
			|| ($id == 0 && !$user->authorise('favorite_listing.edit.created_by', 'com_realestatenow')))
		{
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/favorite_listing.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_realestatenow.favorite_listing.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_realestatenow.favorite_listing.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_realestatenow.favorite_listing.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_favorite_listing'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.favorite_listing.data', array());
