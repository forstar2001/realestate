 ...
 * Images Model
 */
class RealestatenowModelImages extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.filename LIKE '.$search.' OR a.rets_source LIKE '.$search.' OR a.title LIKE '.$search.')');
			}
		}
	...
	public function getExportData($pks)
	{
		...
		if (RealestatenowHelper::checkArray($pks))
		{
			...
			// [Interpretation 8859] From the realestatenow_image table
			$query->from($db->quoteName('#__realestatenow_image', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__realestatenow_image");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__realestatenow_image'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__realestatenow_image'))->set($fields)->where($conditions); 
