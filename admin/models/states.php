...
class RealestatenowModelStates extends JModelList
{
	...
	public function getItems()
	{
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			{
				$access = (JFactory::getUser()->authorise('state.access', 'com_realestatenow.state.' . (int) $item->id) && JFactory::getUser()->authorise('state.access', 'com_realestatenow'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	public function getExportData($pks)
	{
		...
		if (RealestatenowHelper::checkArray($pks))
		{
			...
			// [Interpretation 8859] From the realestatenow_state table
			$query->from($db->quoteName('#__realestatenow_state', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__realestatenow_state");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__realestatenow_state'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
