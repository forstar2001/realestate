...
/**
 * Realestatenow Feature Model
 */
class RealestatenowModelFeature extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.feature';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'feature', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			{
				// [Interpretation 4502] Convert the outdoorfeatures field to an array.
			...
			{
				// [Interpretation 4502] Convert the otherfeatures field to an array.
			...
			{
				// [Interpretation 4502] Convert the appliances field to an array.
			...
			{
				// [Interpretation 4502] Convert the indoorfeatures field to an array.
			...
			{
				// [Interpretation 4502] Convert the communityfeatures field to an array.
			...
			{
				// [Interpretation 4502] Convert the buildingfeatures field to an array.
			...
			if (!empty($item->sewer))
			{
				// [Interpretation 4517] JSON Decode sewer.
				$item->sewer = json_decode($item->sewer);
			}

			if (!empty($item->zoning))
			{
				// [Interpretation 4517] JSON Decode zoning.
				$item->zoning = json_decode($item->zoning);
			}

			if (!empty($item->porchpatio))
			{
				// [Interpretation 4517] JSON Decode porchpatio.
				$item->porchpatio = json_decode($item->porchpatio);
			}

			if (!empty($item->waterresources))
			{
				// [Interpretation 4517] JSON Decode waterresources.
				$item->waterresources = json_decode($item->waterresources);
			}

			if (!empty($item->basementandfoundation))
			{
				// [Interpretation 4517] JSON Decode basementandfoundation.
				$item->basementandfoundation = json_decode($item->basementandfoundation);
			}

			if (!empty($item->roof))
			{
				// [Interpretation 4517] JSON Decode roof.
				$item->roof = json_decode($item->roof);
			}

			if (!empty($item->garagetype))
			{
				// [Interpretation 4517] JSON Decode garagetype.
				$item->garagetype = json_decode($item->garagetype);
			}

			if (!empty($item->frontage))
			{
				// [Interpretation 4517] JSON Decode frontage.
				$item->frontage = json_decode($item->frontage);
			}

			if (!empty($item->heating))
			{
				// [Interpretation 4517] JSON Decode heating.
				$item->heating = json_decode($item->heating);
			}

			if (!empty($item->cooling))
			{
				// [Interpretation 4517] JSON Decode cooling.
				$item->cooling = json_decode($item->cooling);
			}

			if (!empty($item->fencing))
			{
				// [Interpretation 4517] JSON Decode fencing.
				$item->fencing = json_decode($item->fencing);
			}

			if (!empty($item->exteriorfinish))
			{
				// [Interpretation 4517] JSON Decode exteriorfinish.
				$item->exteriorfinish = json_decode($item->exteriorfinish);
			}
			
			if (!empty($item->id))
			{
				$item->tags = new JHelperTags;
				$item->tags->getTagIds($item->id, 'com_realestatenow.feature');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.feature', 'feature', $options);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_realestatenow.feature.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('core.edit.created_by', 'com_realestatenow.feature.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('core.edit.created', 'com_realestatenow.feature.' . (int) $id))
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/feature.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			$user = JFactory::getUser();
			// [Interpretation 11501] The record has been set. Check the record permissions.
			return $user->authorise('core.delete', 'com_realestatenow.feature.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_realestatenow.feature.' . (int) $recordId);
	...
	{
		// [Interpretation 11412] Check specific edit permission then general edit permission.
		$user = JFactory::getUser();

		return $user->authorise('core.edit', 'com_realestatenow.feature.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('core.edit',  'com_realestatenow');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_feature'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.feature.data', array());
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= RealestatenowHelper::getActions('feature');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		// [Interpretation 4564] Set the outdoorfeatures items to data.
		...
		{
			// [Interpretation 4573] Set the empty outdoorfeatures to data
			$data['outdoorfeatures'] = '';
		}

		// [Interpretation 4564] Set the otherfeatures items to data.
		...
		{
			// [Interpretation 4573] Set the empty otherfeatures to data
			$data['otherfeatures'] = '';
		}

		// [Interpretation 4564] Set the appliances items to data.
		...
		{
			// [Interpretation 4573] Set the empty appliances to data
			$data['appliances'] = '';
		}

		// [Interpretation 4564] Set the indoorfeatures items to data.
		...
		{
			// [Interpretation 4573] Set the empty indoorfeatures to data
			$data['indoorfeatures'] = '';
		}

		// [Interpretation 4564] Set the communityfeatures items to data.
		...
		{
			// [Interpretation 4573] Set the empty communityfeatures to data
			$data['communityfeatures'] = '';
		}

		// [Interpretation 4564] Set the buildingfeatures items to data.
		...
		{
			// [Interpretation 4573] Set the empty buildingfeatures to data
			$data['buildingfeatures'] = '';
		}

		// [Interpretation 4583] Set the sewer string to JSON string.
		if (isset($data['sewer']))
		{
			$data['sewer'] = (string) json_encode($data['sewer']);
		}

		// [Interpretation 4583] Set the zoning string to JSON string.
		if (isset($data['zoning']))
		{
			$data['zoning'] = (string) json_encode($data['zoning']);
		}

		// [Interpretation 4583] Set the porchpatio string to JSON string.
		if (isset($data['porchpatio']))
		{
			$data['porchpatio'] = (string) json_encode($data['porchpatio']);
		}

		// [Interpretation 4583] Set the waterresources string to JSON string.
		if (isset($data['waterresources']))
		{
			$data['waterresources'] = (string) json_encode($data['waterresources']);
		}

		// [Interpretation 4583] Set the basementandfoundation string to JSON string.
		if (isset($data['basementandfoundation']))
		{
			$data['basementandfoundation'] = (string) json_encode($data['basementandfoundation']);
		}

		// [Interpretation 4583] Set the roof string to JSON string.
		if (isset($data['roof']))
		{
			$data['roof'] = (string) json_encode($data['roof']);
		}

		// [Interpretation 4583] Set the garagetype string to JSON string.
		if (isset($data['garagetype']))
		{
			$data['garagetype'] = (string) json_encode($data['garagetype']);
		}

		// [Interpretation 4583] Set the frontage string to JSON string.
		if (isset($data['frontage']))
		{
			$data['frontage'] = (string) json_encode($data['frontage']);
		}

		// [Interpretation 4583] Set the heating string to JSON string.
		if (isset($data['heating']))
		{
			$data['heating'] = (string) json_encode($data['heating']);
		}

		// [Interpretation 4583] Set the cooling string to JSON string.
		if (isset($data['cooling']))
		{
			$data['cooling'] = (string) json_encode($data['cooling']);
		}

		// [Interpretation 4583] Set the fencing string to JSON string.
		if (isset($data['fencing']))
		{
			$data['fencing'] = (string) json_encode($data['fencing']);
		}

		// [Interpretation 4583] Set the exteriorfinish string to JSON string.
		if (isset($data['exteriorfinish']))
		{
			$data['exteriorfinish'] = (string) json_encode($data['exteriorfinish']);
		}
