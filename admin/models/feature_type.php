...
/**
 * Realestatenow Feature_type Model
 */
class RealestatenowModelFeature_type extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.feature_type';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'feature_type', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.feature_type');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.feature_type', 'feature_type', $options);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_realestatenow.feature_type.' . (int) $id))
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/feature_type.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_realestatenow.feature_type.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_realestatenow.feature_type.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_realestatenow.feature_type.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_feature_type'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.feature_type.data', array());
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			list($this->table->featurename, $this->table->alias) = $this->_generateNewTitle($this->table->alias, $this->table->featurename);
	...
	public function save($data)
	{
		...
		// [Interpretation 5872] Alter the featurename for save as copy
		if ($input->get('task') === 'save2copy')
		{
			$origTable = clone $this->getTable();
			$origTable->load($input->getInt('id'));

			if ($data['featurename'] == $origTable->featurename)
			{
				list($featurename, $alias) = $this->_generateNewTitle($data['alias'], $data['featurename']);
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$data['alias'] = JFilterOutput::stringURLUnicodeSlug($data['featurename']);
				}
				else
				{
					$data['alias'] = JFilterOutput::stringURLSafe($data['featurename']);
				}

				$table = JTable::getInstance('feature_type', 'realestatenowTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_REALESTATENOW_FEATURE_TYPE_SAVE_WARNING');
				}
