...
/**
 * Realestatenow Country Model
 */
class RealestatenowModelCountry extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.country';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'country', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.country');
			}
		}
		$this->countryidvvvv = $item->id;
	 ...
	 */
	public function getVvxproperty_listings()
	{
		...
		// [Interpretation 8502] Filter by countryidvvvv global.
		$countryidvvvv = $this->countryidvvvv;
		if (is_numeric($countryidvvvv ))
		{
			$query->where('a.countryid = ' . (int) $countryidvvvv );
		}
		elseif (is_string($countryidvvvv))
		{
			$query->where('a.countryid = ' . $db->quote($countryidvvvv));
		}
		else
		{
			$query->where('a.countryid = -5');
		}
		...
		if ($db->getNumRows())
		{
			...
			if (RealestatenowHelper::checkArray($items))
			{
				foreach ($items as $nr => &$item)
				{
					...
					$item->featured = $this->selectionTranslationVvxproperty_listings($item->featured, 'featured');
				}
			}
	 ...
	 */
	public function selectionTranslationVvxproperty_listings($value,$name)
	{
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.country', 'country', $options);
		...
		if ($id != 0 && (!$user->authorise('country.edit.state', 'com_realestatenow.country.' . (int) $id))
			|| ($id == 0 && !$user->authorise('country.edit.state', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('country.edit.created_by', 'com_realestatenow.country.' . (int) $id))
			|| ($id == 0 && !$user->authorise('country.edit.created_by', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('country.edit.created', 'com_realestatenow.country.' . (int) $id))
			|| ($id == 0 && !$user->authorise('country.edit.created', 'com_realestatenow')))
		{
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/country.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('country.delete', 'com_realestatenow.country.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('country.edit.state', 'com_realestatenow.country.' . (int) $recordId);
		...
		return $user->authorise('country.edit.state', 'com_realestatenow');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('country.edit', 'com_realestatenow.country.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('country.edit',  'com_realestatenow');
	}
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_country'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.country.data', array());
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= RealestatenowHelper::getActions('country');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('country', 'realestatenowTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_REALESTATENOW_COUNTRY_SAVE_WARNING');
				}
