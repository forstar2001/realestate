 ...
 * Rent_types Model
 */
class RealestatenowModelRent_types extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__realestatenow_rent_type', 'a'));
	...
	public function getExportData($pks)
	{
		...
		if (RealestatenowHelper::checkArray($pks))
		{
			...
			// [Interpretation 8859] From the realestatenow_rent_type table
			$query->from($db->quoteName('#__realestatenow_rent_type', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__realestatenow_rent_type");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__realestatenow_rent_type'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__realestatenow_rent_type'))->set($fields)->where($conditions); 
