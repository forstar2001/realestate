...
/**
 * Realestatenow Agent Model
 */
class RealestatenowModelAgent extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.agent';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'agent', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.agent');
			}
		}
		$this->agentvvvz = $item->id;
	 ...
	 */
	public function getVwjproperty_listings()
	{
		// [Interpretation 8475] Get the user object.
		$user = JFactory::getUser();
		// [Interpretation 8477] Create a new query object.
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);

		// [Interpretation 8480] Select some fields
		$query->select('a.*');
		$query->select($db->quoteName('c.title','category_title'));

		// [Interpretation 8487] From the realestatenow_property table
		...
		// [Interpretation 8502] Filter by agentvvvz global.
		$agentvvvz = $this->agentvvvz;
		if (is_numeric($agentvvvz ))
		{
			$query->where('a.agent = ' . (int) $agentvvvz );
		}
		elseif (is_string($agentvvvz))
		{
			$query->where('a.agent = ' . $db->quote($agentvvvz));
		}
		else
		{
			$query->where('a.agent = -5');
		}

		// [Interpretation 8561] Join over the asset groups.
		$query->select('ag.title AS access_level');
		$query->join('LEFT', '#__viewlevels AS ag ON ag.id = a.access');
		// [Interpretation 8564] Filter by access level.
		if ($access = $this->getState('filter.access'))
		{
			$query->where('a.access = ' . (int) $access);
		}
		// [Interpretation 8569] Implement View Level Access
		...
		// [Interpretation 8576] Order the results by ordering
		$query->order('a.published  ASC');
		$query->order('a.ordering  ASC');

		// [Interpretation 8579] Load the items
		$db->setQuery($query);
		$db->execute();
		if ($db->getNumRows())
		{
			...
			if (RealestatenowHelper::checkArray($items))
			{
				...
				{
					$access = (JFactory::getUser()->authorise('property.access', 'com_realestatenow.property.' . (int) $item->id) && JFactory::getUser()->authorise('property.access', 'com_realestatenow'));
					if (!$access)
					{
						unset($items[$nr]);
						continue;
			...
			// [Interpretation 12607] set selection value to a translatable value
			if (RealestatenowHelper::checkArray($items))
			{
				foreach ($items as $nr => &$item)
				{
					// [Interpretation 12614] convert featured
					$item->featured = $this->selectionTranslationVwjproperty_listings($item->featured, 'featured');
				}
			}
	 ...
	 */
	public function selectionTranslationVwjproperty_listings($value,$name)
	{
		// [Interpretation 12640] Array of featured language strings
		if ($name === 'featured')
		{
			$featuredArray = array(
				1 => 'COM_REALESTATENOW_PROPERTY_YES',
				0 => 'COM_REALESTATENOW_PROPERTY_NO'
			);
			// [Interpretation 12671] Now check if value is found in this array
			if (isset($featuredArray[$value]) && RealestatenowHelper::checkString($featuredArray[$value]))
			{
				return $featuredArray[$value];
			}
		}
		return $value;
	}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.agent', 'agent', $options);
		...
		if ($id != 0 && (!$user->authorise('agent.edit.state', 'com_realestatenow.agent.' . (int) $id))
			|| ($id == 0 && !$user->authorise('agent.edit.state', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('agent.edit.created_by', 'com_realestatenow.agent.' . (int) $id))
			|| ($id == 0 && !$user->authorise('agent.edit.created_by', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('agent.edit.created', 'com_realestatenow.agent.' . (int) $id))
			|| ($id == 0 && !$user->authorise('agent.edit.created', 'com_realestatenow')))
		{
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/agent.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('agent.delete', 'com_realestatenow.agent.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('agent.edit.state', 'com_realestatenow.agent.' . (int) $recordId);
			if (!$permission && !is_null($permission))
			{
				return false;
			}
		}
		// [Interpretation 11606] In the absense of better information, revert to the component permissions.
		return $user->authorise('agent.edit.state', 'com_realestatenow');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('agent.edit', 'com_realestatenow.agent.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('agent.edit',  'com_realestatenow');
	}
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_agent'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.agent.data', array());
	...
	{
		// [Interpretation 10292] check if the not_required field is set
		if (RealestatenowHelper::checkString($data['not_required']))
		{
			$requiredFields = (array) explode(',',(string) $data['not_required']);
			$requiredFields = array_unique($requiredFields);
			// [Interpretation 10297] now change the required field attributes value
			foreach ($requiredFields as $requiredField)
			{
				// [Interpretation 10300] make sure there is a string value
				if (RealestatenowHelper::checkString($requiredField))
				{
					// [Interpretation 10303] change to false
					$form->setFieldAttribute($requiredField, 'required', 'false');
					// [Interpretation 10305] also clear the data set
					$data[$requiredField] = '';
				}
			}
		}
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('agent.create') && !$this->canDo->get('agent.batch'))
		{
			return false;
		}
		...
		elseif (isset($values['published']) && !$this->canDo->get('agent.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		{
			// [Interpretation 5687] move the category value to correct field name
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('agent.edit', $contexts[$pk]))
			{
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('agent.edit') && !$this->canDo->get('agent.batch'))
		{
		...
		if (isset($values['published']) && !$this->canDo->get('agent.edit.state'))
		{
		...
		elseif (isset($values['category']) && (int) $values['category'] > 0)
		{
			// [Interpretation 5476] move the category value to correct field name
			$values['catid'] = $values['category'];
			unset($values['category']);
		}
		elseif (isset($values['category']))
	...
	public function save($data)
	{
		...
		// [Interpretation 5872] Alter the name for save as copy
		...
		// [Interpretation 5916] Automatic handling of alias for empty fields
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('agent', 'realestatenowTable');

				if ($table->load(array('alias' => $data['alias'], 'catid' => $data['catid'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_REALESTATENOW_AGENT_SAVE_WARNING');
				}
	...
	protected function _generateNewTitle($alias, $title = null)
	{
		...
		// [Interpretation 5993] Alter the title/s & alias
		$table = $this->getTable();

		while ($table->load(array('alias' => $alias)))
		{
			// [Interpretation 5997] Check if this is an array of titles
			if (RealestatenowHelper::checkArray($title))
			{
			...
			// [Interpretation 6005] Make sure we have a title
			elseif ($title)
			{
		...
		// [Interpretation 6012] Check if this is an array of titles
		if (RealestatenowHelper::checkArray($title))
		{
			$title[] = $alias;
			return $title;
		}
		// [Interpretation 6018] Make sure we have a title
		elseif ($title)
		{
			return array($title, $alias);
		}
		// [Interpretation 6023] We only had an alias
		return $alias;
	}
}
