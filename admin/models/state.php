...
/**
 * Realestatenow State Model
 */
class RealestatenowModelState extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.state';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'state', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.state');
			}
		}
		$this->stateidvvvw = $item->id;
	 ...
	 */
	public function getVwaproperty_listings()
	{
		...
		// [Interpretation 8502] Filter by stateidvvvw global.
		$stateidvvvw = $this->stateidvvvw;
		if (is_numeric($stateidvvvw ))
		{
			$query->where('a.stateid = ' . (int) $stateidvvvw );
		}
		elseif (is_string($stateidvvvw))
		{
			$query->where('a.stateid = ' . $db->quote($stateidvvvw));
		}
		else
		{
			$query->where('a.stateid = -5');
		}
		...
		if ($db->getNumRows())
		{
			...
			if (RealestatenowHelper::checkArray($items))
			{
				foreach ($items as $nr => &$item)
				{
					...
					$item->featured = $this->selectionTranslationVwaproperty_listings($item->featured, 'featured');
				}
			}
	 ...
	 */
	public function selectionTranslationVwaproperty_listings($value,$name)
	{
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.state', 'state', $options);
		...
		if ($id != 0 && (!$user->authorise('state.edit.state', 'com_realestatenow.state.' . (int) $id))
			|| ($id == 0 && !$user->authorise('state.edit.state', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('state.edit.created_by', 'com_realestatenow.state.' . (int) $id))
			|| ($id == 0 && !$user->authorise('state.edit.created_by', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('core.edit.created', 'com_realestatenow.state.' . (int) $id))
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/state.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('state.delete', 'com_realestatenow.state.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('state.edit.state', 'com_realestatenow.state.' . (int) $recordId);
		...
		return $user->authorise('state.edit.state', 'com_realestatenow');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('state.edit', 'com_realestatenow.state.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('state.edit',  'com_realestatenow');
	}
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_state'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.state.data', array());
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= RealestatenowHelper::getActions('state');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('state', 'realestatenowTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_REALESTATENOW_STATE_SAVE_WARNING');
				}
