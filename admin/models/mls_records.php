 ...
 * Mls_records Model
 */
class RealestatenowModelMls_records extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__realestatenow_mls', 'a'));
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.name LIKE '.$search.' OR a.mls_disclaimer LIKE '.$search.')');
			}
		}
	...
	public function getExportData($pks)
	{
		...
		if (RealestatenowHelper::checkArray($pks))
		{
			...
			// [Interpretation 8859] From the realestatenow_mls table
			$query->from($db->quoteName('#__realestatenow_mls', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__realestatenow_mls");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__realestatenow_mls'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__realestatenow_mls'))->set($fields)->where($conditions); 
