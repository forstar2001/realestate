...
/**
 * Favorite_listings Model
 */
class RealestatenowModelFavorite_listings extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$propertyid = $this->getUserStateFromRequest($this->context . '.filter.propertyid', 'filter_propertyid');
		$this->setState('filter.propertyid', $propertyid);
	...
	protected function getListQuery()
	{
		...
		$query->select($db->quoteName('g.name','propertyid_name'));
		$query->join('LEFT', $db->quoteName('#__realestatenow_property', 'g') . ' ON (' . $db->quoteName('a.propertyid') . ' = ' . $db->quoteName('g.id') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.uid LIKE '.$search.' OR a.propertyid LIKE '.$search.' OR g.name LIKE '.$search.')');
			}
		}

		// [Interpretation 9255] Filter by Uid.
		if ($uid = $this->getState('filter.uid'))
		{
			$query->where('a.uid = ' . $db->quote($db->escape($uid)));
		}
		// [Interpretation 9247] Filter by propertyid.
		if ($propertyid = $this->getState('filter.propertyid'))
		{
			$query->where('a.propertyid = ' . $db->quote($db->escape($propertyid)));
		}
	...
	public function getExportData($pks)
	{
		...
		if (RealestatenowHelper::checkArray($pks))
		{
			...
			// [Interpretation 8859] From the realestatenow_favorite_listing table
			$query->from($db->quoteName('#__realestatenow_favorite_listing', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__realestatenow_favorite_listing");
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.propertyid');
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__realestatenow_favorite_listing'))->set($fields)->where($conditions); 
