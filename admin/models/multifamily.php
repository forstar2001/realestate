...
/**
 * Realestatenow Multifamily Model
 */
class RealestatenowModelMultifamily extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.multifamily';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'multifamily', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->unitfeatures))
			{
				// [Interpretation 4502] Convert the unitfeatures field to an array.
				$unitfeatures = new Registry;
				$unitfeatures->loadString($item->unitfeatures);
				$item->unitfeatures = $unitfeatures->toArray();
			}

			if (!empty($item->commonareas))
			{
				// [Interpretation 4502] Convert the commonareas field to an array.
			...
			{
				// [Interpretation 4502] Convert the unitdetails field to an array.
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.multifamily');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.multifamily', 'multifamily', $options);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_realestatenow.multifamily.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('core.edit.created_by', 'com_realestatenow.multifamily.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('core.edit.created', 'com_realestatenow.multifamily.' . (int) $id))
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/multifamily.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_realestatenow.multifamily.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_realestatenow.multifamily.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('core.edit', 'com_realestatenow.multifamily.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('core.edit',  'com_realestatenow');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_multifamily'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.multifamily.data', array());
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= RealestatenowHelper::getActions('multifamily');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		// [Interpretation 4564] Set the unitfeatures items to data.
		if (isset($data['unitfeatures']) && is_array($data['unitfeatures']))
		{
			$unitfeatures = new JRegistry;
			$unitfeatures->loadArray($data['unitfeatures']);
			$data['unitfeatures'] = (string) $unitfeatures;
		}
		elseif (!isset($data['unitfeatures']))
		{
			// [Interpretation 4573] Set the empty unitfeatures to data
			$data['unitfeatures'] = '';
		}

		// [Interpretation 4564] Set the commonareas items to data.
		...
		{
			// [Interpretation 4573] Set the empty commonareas to data
			$data['commonareas'] = '';
		}

		// [Interpretation 4564] Set the unitdetails items to data.
		...
		{
			// [Interpretation 4573] Set the empty unitdetails to data
			$data['unitdetails'] = '';
		}
