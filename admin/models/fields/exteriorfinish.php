...
/**
 * Exteriorfinish Form Field class for the Realestatenow component
 */
class JFormFieldExteriorfinish extends JFormFieldList
{
	/**
	 * The exteriorfinish field type.
	 *
	 * @var		string
	 */
	public $type = 'exteriorfinish';
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.featurename'),array('id','exteriorfinish_featurename')));
		$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
		$query->where($db->quoteName('a.featuretype').'=7')->andWhere($db->quoteName('a.published').'=1');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->exteriorfinish_featurename);
			}
		}
		return $options;
	}
