...
/**
 * Frontage Form Field class for the Realestatenow component
 */
class JFormFieldFrontage extends JFormFieldList
{
	/**
	 * The frontage field type.
	 *
	 * @var		string
	 */
	public $type = 'frontage';
...
$query->select($db->quoteName(array('a.id','a.featurename'),array('id','frontage_featurename')));
$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
$query->where($db->quoteName('a.featuretype').'=5')->andWhere($db->quoteName('a.published').'=1');
...
if ($items)
{
	...
	{
		$options[] = JHtml::_('select.option', $item->id, $item->frontage_featurename);
	}
}
