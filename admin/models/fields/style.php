...
/**
 * Style Form Field class for the Realestatenow component
 */
class JFormFieldStyle extends JFormFieldList
{
	/**
	 * The style field type.
	 *
	 * @var		string
	 */
	public $type = 'style';
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.featurename'),array('id','style_featurename')));
		$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
		$query->where($db->quoteName('a.featuretype').'=8')->andWhere($db->quoteName('a.published').'=1');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->style_featurename);
			}
		}
		return $options;
	}
