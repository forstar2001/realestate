...
/**
 * Cooling Form Field class for the Realestatenow component
 */
class JFormFieldCooling extends JFormFieldList
{
	/**
	 * The cooling field type.
	 *
	 * @var		string
	 */
	public $type = 'cooling';
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.featurename'),array('id','cooling_featurename')));
		$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
		$query->where($db->quoteName('a.featuretype').'=11')->andWhere($db->quoteName('a.published').'=1');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->cooling_featurename);
			}
		}
		return $options;
	}
