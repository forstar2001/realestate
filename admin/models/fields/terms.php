...
/**
 * Terms Form Field class for the Realestatenow component
 */
class JFormFieldTerms extends JFormFieldList
{
	/**
	 * The terms field type.
	 *
	 * @var		string
	 */
	public $type = 'terms';
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.featurename'),array('id','terms_featurename')));
		$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
		$query->where($db->quoteName('a.featuretype').'=14')->andWhere($db->quoteName('a.published').'=1');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->terms_featurename);
			}
		}
		return $options;
	}
