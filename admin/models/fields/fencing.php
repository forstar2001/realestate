...
/**
 * Fencing Form Field class for the Realestatenow component
 */
class JFormFieldFencing extends JFormFieldList
{
	/**
	 * The fencing field type.
	 *
	 * @var		string
	 */
	public $type = 'fencing';
...
$query->select($db->quoteName(array('a.id','a.featurename'),array('id','fencing_featurename')));
$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
$query->where($db->quoteName('a.featuretype').'=2')->andWhere($db->quoteName('a.published').'=1');
...
if ($items)
{
	...
	{
		$options[] = JHtml::_('select.option', $item->id, $item->fencing_featurename);
	}
}
