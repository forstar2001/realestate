...
/**
 * Transtype Form Field class for the Realestatenow component
 */
class JFormFieldTranstype extends JFormFieldList
{
	/**
	 * The transtype field type.
	 *
	 * @var		string
	 */
	...
	protected function getInput()
	{
		...
		if ($setButton === 'true')
		{
			...
			// [Fields 3060] only add if user allowed to create transaction_type
			if ($user->authorise('core.create', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$buttonName.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_REALESTATENOW_CREATE_NEW_S', $buttonNamee).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_realestatenow&amp;view=transaction_type&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 3081] only add if user allowed to edit transaction_type
			if (($buttonName === 'transaction_type' || $buttonName === 'transaction_types')  && $user->authorise('core.edit', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$script[] = "
					...
					function ".$buttonName."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_realestatenow&view=transaction_types&task=transaction_type.edit&id='+value+'".$refJ."';
			...
			// [Fields 3127] check if button was created for transaction_type field.
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.name'),array('id','trans_type_name')));
		...
		if ($items)
		{
			$options[] = JHtml::_('select.option', '', 'Select a transaction type');
			foreach($items as $item)
			{
				$options[] = JHtml::_('select.option', $item->id, $item->trans_type_name);
			}
		}
		return $options;
	}
