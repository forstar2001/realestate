...
/**
 * Renttype Form Field class for the Realestatenow component
 */
class JFormFieldRenttype extends JFormFieldList
{
	/**
	 * The renttype field type.
	 *
	 * @var		string
	 */
	...
	protected function getInput()
	{
		...
		if ($setButton === 'true')
		{
			...
			// [Fields 3060] only add if user allowed to create renttype
			if ($user->authorise('core.create', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$buttonName.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_REALESTATENOW_CREATE_NEW_S', $buttonNamee).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_realestatenow&amp;view=renttype&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 3081] only add if user allowed to edit renttype
			if (($buttonName === 'renttype' || $buttonName === 'renttypes')  && $user->authorise('core.edit', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$script[] = "
					...
					function ".$buttonName."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_realestatenow&view=renttypes&task=renttype.edit&id='+value+'".$refJ."';
			...
			// [Fields 3127] check if button was created for renttype field.
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('f.id','f.name'),array('id','rent_type_name')));
		$query->from($db->quoteName('#__realestatenow_rent_type', 'f'));
		...
		if ($items)
		{
			$options[] = JHtml::_('select.option', '', 'Select a rental type');
			foreach($items as $item)
			{
				$options[] = JHtml::_('select.option', $item->id, $item->rent_type_name);
			}
		}
		return $options;
	}
