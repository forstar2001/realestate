...
/**
 * Heating Form Field class for the Realestatenow component
 */
class JFormFieldHeating extends JFormFieldList
{
	/**
	 * The heating field type.
	 *
	 * @var		string
	 */
	public $type = 'heating';
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.featurename'),array('id','heating_featurename')));
		$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
		$query->where($db->quoteName('a.featuretype').'=10')->andWhere($db->quoteName('a.published').'=1');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->heating_featurename);
			}
		}
		return $options;
	}
