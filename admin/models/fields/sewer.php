...
/**
 * Sewer Form Field class for the Realestatenow component
 */
class JFormFieldSewer extends JFormFieldList
{
	/**
	 * The sewer field type.
	 *
	 * @var		string
	 */
	public $type = 'sewer';
...
$query->select($db->quoteName(array('a.id','a.featurename'),array('id','sewer_featurename')));
$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
$query->where($db->quoteName('a.featuretype').'=12')->andWhere($db->quoteName('a.published').'=1');
...
if ($items)
{
	...
	{
		$options[] = JHtml::_('select.option', $item->id, $item->sewer_featurename);
	}
}
