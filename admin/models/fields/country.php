...
/**
 * Country Form Field class for the Realestatenow component
 */
class JFormFieldCountry extends JFormFieldList
{
	...
	protected function getInput()
	{
		...
		if ($setButton === 'true')
		{
			...
			// [Fields 3060] only add if user allowed to create country
			if ($user->authorise('country.create', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$buttonName.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_REALESTATENOW_CREATE_NEW_S', $buttonNamee).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_realestatenow&amp;view=country&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 3081] only add if user allowed to edit country
			if (($buttonName === 'country' || $buttonName === 'countries') && $user->authorise('country.edit', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$script[] = "
					...
					function ".$buttonName."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_realestatenow&view=countries&task=country.edit&id='+value+'".$refJ."';
			...
			// [Fields 3127] check if button was created for country field.
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('c.id','c.name'),array('id','countryid_name')));
		$query->from($db->quoteName('#__realestatenow_country', 'c'));
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->countryid_name);
			}
		}
		return $options;
	}
