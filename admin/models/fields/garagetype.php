...
/**
 * Garagetype Form Field class for the Realestatenow component
 */
class JFormFieldGaragetype extends JFormFieldList
{
	/**
	 * The garagetype field type.
	 *
	 * @var		string
	 */
	public $type = 'garagetype';
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.featurename'),array('id','garagetype_featurename')));
		$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
		$query->where($db->quoteName('a.featuretype').'=3')->andWhere($db->quoteName('a.published').'=1');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->garagetype_featurename);
			}
		}
		return $options;
	}
