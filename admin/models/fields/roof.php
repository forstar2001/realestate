...
/**
 * Roof Form Field class for the Realestatenow component
 */
class JFormFieldRoof extends JFormFieldList
{
	/**
	 * The roof field type.
	 *
...
$query->select($db->quoteName(array('a.id','a.featurename'),array('id','roof_featurename')));
$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
$query->where($db->quoteName('a.featuretype').'=6')->andWhere($db->quoteName('a.published').'=1');
...
if ($items)
{
	...
	{
		$options[] = JHtml::_('select.option', $item->id, $item->roof_featurename);
	}
}
