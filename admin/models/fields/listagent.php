...
/**
 * Listagent Form Field class for the Realestatenow component
 */
class JFormFieldListagent extends JFormFieldList
{
	/**
	 * The listagent field type.
	 *
	 * @var		string
	 */
	public $type = 'listagent';
	...
	protected function getInput()
	{
		...
		if ($setButton === 'true')
		{
			...
			// [Fields 3060] only add if user allowed to create agent
			if ($user->authorise('agent.create', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$buttonName.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_REALESTATENOW_CREATE_NEW_S', $buttonNamee).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_realestatenow&amp;view=agent&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 3081] only add if user allowed to edit agent
			if (($buttonName === 'agent' || $buttonName === 'agents') && $user->authorise('agent.edit', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$script[] = "
					...
					function ".$buttonName."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_realestatenow&view=agents&task=agent.edit&id='+value+'".$refJ."';
			...
			// [Fields 3127] check if button was created for agent field.
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.name'),array('id','agent_name')));
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->agent_name);
			}
		}
		return $options;
	}
