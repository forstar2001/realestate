...
/**
 * Basement Form Field class for the Realestatenow component
 */
class JFormFieldBasement extends JFormFieldList
{
	/**
	 * The basement field type.
	 *
	 * @var		string
	 */
	public $type = 'basement';
...
$query->select($db->quoteName(array('a.id','a.featurename'),array('id','basementandfoundation_featurename')));
$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
$query->where($db->quoteName('a.featuretype').'=1')->andWhere($db->quoteName('a.published').'=1');
...
if ($items)
{
	...
	{
		$options[] = JHtml::_('select.option', $item->id, $item->basementandfoundation_featurename);
	}
}
