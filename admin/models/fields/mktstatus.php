...
/**
 * Mktstatus Form Field class for the Realestatenow component
 */
class JFormFieldMktstatus extends JFormFieldList
{
	/**
	 * The mktstatus field type.
	 *
	 * @var		string
	 */
	public $type = 'mktstatus';
	...
	protected function getInput()
	{
		...
		if ($setButton === 'true')
		{
			...
			// [Fields 3060] only add if user allowed to create market_status
			if ($user->authorise('core.create', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$buttonName.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_REALESTATENOW_CREATE_NEW_S', $buttonNamee).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_realestatenow&amp;view=market_status&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 3081] only add if user allowed to edit market_status
			if (($buttonName === 'market_status' || $buttonName === 'market_statuses')  && $user->authorise('core.edit', 'com_realestatenow') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$script[] = "
					...
					function ".$buttonName."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_realestatenow&view=market_statuses&task=market_status.edit&id='+value+'".$refJ."';
			...
			// [Fields 3127] check if button was created for market_status field.
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.name'),array('id','mkt_stats_name')));
		...
		if ($items)
		{
			$options[] = JHtml::_('select.option', '', 'Select a market status');
			foreach($items as $item)
			{
				$options[] = JHtml::_('select.option', $item->id, $item->mkt_stats_name);
			}
		}
		return $options;
	}
