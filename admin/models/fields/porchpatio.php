...
/**
 * Porchpatio Form Field class for the Realestatenow component
 */
class JFormFieldPorchpatio extends JFormFieldList
{
	/**
	 * The porchpatio field type.
	 *
	 * @var		string
	 */
	public $type = 'porchpatio';
	...
	protected function getOptions()
	{
		...
		$query->select($db->quoteName(array('a.id','a.featurename'),array('id','porchpatio_featurename')));
		$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
		$query->where($db->quoteName('a.featuretype').'=4')->andWhere($db->quoteName('a.published').'=1');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->porchpatio_featurename);
			}
		}
		return $options;
	}
