...
/**
 * Zoning Form Field class for the Realestatenow component
 */
class JFormFieldZoning extends JFormFieldList
{
	/**
	 * The zoning field type.
	 *
	 * @var		string
	 */
	public $type = 'zoning';
...
$query->select($db->quoteName(array('a.id','a.featurename'),array('id','zoning_featurename')));
$query->from($db->quoteName('#__realestatenow_feature_type', 'a'));
$query->where($db->quoteName('a.featuretype').'=13')->andWhere($db->quoteName('a.published').'=1');
...
if ($items)
{
	...
	{
		$options[] = JHtml::_('select.option', $item->id, $item->zoning_featurename);
	}
}
