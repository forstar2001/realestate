...
/**
 * Realestatenow Rental Model
 */
class RealestatenowModelRental extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.rental';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'rental', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.rental');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.rental', 'rental', $options);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_realestatenow.rental.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('core.edit.created_by', 'com_realestatenow.rental.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('core.edit.created', 'com_realestatenow.rental.' . (int) $id))
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/rental.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_realestatenow.rental.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_realestatenow.rental.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('core.edit', 'com_realestatenow.rental.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('core.edit',  'com_realestatenow');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_rental'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.rental.data', array());
