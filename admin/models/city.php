...
/**
 * Realestatenow City Model
 */
class RealestatenowModelCity extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.city';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'city', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.city');
			}
		}
		$this->cityidvvvx = $item->id;
	 ...
	 */
	public function getVwdproperty_listings()
	{
		...
		// [Interpretation 8502] Filter by cityidvvvx global.
		$cityidvvvx = $this->cityidvvvx;
		if (is_numeric($cityidvvvx ))
		{
			$query->where('a.cityid = ' . (int) $cityidvvvx );
		}
		elseif (is_string($cityidvvvx))
		{
			$query->where('a.cityid = ' . $db->quote($cityidvvvx));
		}
		else
		{
			$query->where('a.cityid = -5');
		}
		...
		if ($db->getNumRows())
		{
			...
			if (RealestatenowHelper::checkArray($items))
			{
				foreach ($items as $nr => &$item)
				{
					...
					$item->featured = $this->selectionTranslationVwdproperty_listings($item->featured, 'featured');
				}
			}
	 ...
	 */
	public function selectionTranslationVwdproperty_listings($value,$name)
	{
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.city', 'city', $options);
		...
		if ($id != 0 && (!$user->authorise('city.edit.state', 'com_realestatenow.city.' . (int) $id))
			|| ($id == 0 && !$user->authorise('city.edit.state', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('city.edit.created_by', 'com_realestatenow.city.' . (int) $id))
			|| ($id == 0 && !$user->authorise('city.edit.created_by', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('city.edit.created', 'com_realestatenow.city.' . (int) $id))
			|| ($id == 0 && !$user->authorise('city.edit.created', 'com_realestatenow')))
		{
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/city.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('city.delete', 'com_realestatenow.city.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('city.edit.state', 'com_realestatenow.city.' . (int) $recordId);
		...
		return $user->authorise('city.edit.state', 'com_realestatenow');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('city.edit', 'com_realestatenow.city.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('city.edit',  'com_realestatenow');
	}
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_city'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.city.data', array());
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('city', 'realestatenowTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_REALESTATENOW_CITY_SAVE_WARNING');
				}
