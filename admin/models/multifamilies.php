 ...
 * Multifamilies Model
 */
class RealestatenowModelMultifamilies extends JModelList
{
	...
	public function getItems()
	{
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			{
				$access = (JFactory::getUser()->authorise('multifamily.access', 'com_realestatenow.multifamily.' . (int) $item->id) && JFactory::getUser()->authorise('multifamily.access', 'com_realestatenow'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__realestatenow_multifamily', 'a'));
	...
	public function getExportData($pks)
	{
		...
		if (RealestatenowHelper::checkArray($pks))
		{
			...
			// [Interpretation 8859] From the realestatenow_multifamily table
			$query->from($db->quoteName('#__realestatenow_multifamily', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__realestatenow_multifamily");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__realestatenow_multifamily'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__realestatenow_multifamily'))->set($fields)->where($conditions); 
