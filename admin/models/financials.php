 ...
 * Financials Model
 */
class RealestatenowModelFinancials extends JModelList
{
	...
	public function getItems()
	{
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			{
				$access = (JFactory::getUser()->authorise('financial.access', 'com_realestatenow.financial.' . (int) $item->id) && JFactory::getUser()->authorise('financial.access', 'com_realestatenow'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__realestatenow_financial', 'a'));
	...
	public function getExportData($pks)
	{
		...
		if (RealestatenowHelper::checkArray($pks))
		{
			...
			// [Interpretation 8859] From the realestatenow_financial table
			$query->from($db->quoteName('#__realestatenow_financial', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__realestatenow_financial");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__realestatenow_financial'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__realestatenow_financial'))->set($fields)->where($conditions); 
