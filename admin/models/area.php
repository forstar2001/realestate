...
/**
 * Realestatenow Area Model
 */
class RealestatenowModelArea extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.area';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'area', $prefix = 'RealestatenowTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.area');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.area', 'area', $options);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_realestatenow.area.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('core.edit.created_by', 'com_realestatenow.area.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('core.edit.created', 'com_realestatenow.area.' . (int) $id))
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/area.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_realestatenow.area.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_realestatenow.area.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('core.edit', 'com_realestatenow.area.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('core.edit',  'com_realestatenow');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_area'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.area.data', array());
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= RealestatenowHelper::getActions('area');
		$this->batchSet			= true;
