 ...
 */
class RealestatenowModelProperty extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_realestatenow.property';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'property', $prefix = 'RealestatenowTable', $config = array())
	{
...
/***[INSERTED$$$$]***//*334*/
    function getlistingids($view,$ids){

		$db = JFactory::getDBO();
		 $query = "SELECT id FROM #__realestatenow_$view WHERE propid = '$ids' order by propid desc LIMIT 1";
		$db->setQuery($query);
		$rows = $db->loadObjectList();
		if(count($rows) > 0){
			$itemrow = $rows[0];
			$on_frontpage = $itemrow->id;

			if($on_frontpage != ''){
				return '&id='.$on_frontpage;
			}
		}
...
/***[/INSERTED$$$$]***/
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			{
				// [Interpretation 4502] Convert the openhouseinfo field to an array.
			...
			if (!empty($item->style))
			{
				// [Interpretation 4517] JSON Decode style.
            ...
            $JDbQuery->from($this->_db->quoteName('#__realestatenow_image', 'j'));
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_realestatenow.property');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_realestatenow.property', 'property', $options);
		...
		if ($id != 0 && (!$user->authorise('property.edit.state', 'com_realestatenow.property.' . (int) $id))
			|| ($id == 0 && !$user->authorise('property.edit.state', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('property.edit.created_by', 'com_realestatenow.property.' . (int) $id))
			|| ($id == 0 && !$user->authorise('property.edit.created_by', 'com_realestatenow')))
		{
		...
		if ($id != 0 && (!$user->authorise('property.edit.created', 'com_realestatenow.property.' . (int) $id))
			|| ($id == 0 && !$user->authorise('property.edit.created', 'com_realestatenow')))
		{
	...
	{
		return 'administrator/components/com_realestatenow/models/forms/property.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('property.delete', 'com_realestatenow.property.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('property.edit.state', 'com_realestatenow.property.' . (int) $recordId);
		...
		return $user->authorise('property.edit.state', 'com_realestatenow');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('property.edit', 'com_realestatenow.property.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('property.edit',  'com_realestatenow');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__realestatenow_property'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_realestatenow.edit.property.data', array());
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('property.create') && !$this->canDo->get('property.batch'))
		{
			return false;
		}
		...
		elseif (isset($values['published']) && !$this->canDo->get('property.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('property.edit', $contexts[$pk]))
			{
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('property.edit') && !$this->canDo->get('property.batch'))
		{
		...
		if (isset($values['published']) && !$this->canDo->get('property.edit.state'))
		{
	...
	public function save($data)
	{
		...
		// [Interpretation 4564] Set the openhouseinfo items to data.
		...
		{
			// [Interpretation 4573] Set the empty openhouseinfo to data
			$data['openhouseinfo'] = '';
		}

		// [Interpretation 4583] Set the style string to JSON string.
		if (isset($data['style']))
		{
			$data['style'] = (string) json_encode($data['style']);
		}
...
//Custom PHP Save Method - Alias modifications
	...
	protected function _generateNewTitle($title)
	{
		...
		// [Interpretation 6041] Alter the title
		$table = $this->getTable();
