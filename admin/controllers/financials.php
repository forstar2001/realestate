 ...
 */
class RealestatenowControllerFinancials extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_REALESTATENOW_FINANCIALS';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Financial', $prefix = 'RealestatenowModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('financial.export', 'com_realestatenow') && $user->authorise('core.export', 'com_realestatenow'))
		{
			...
			$model = $this->getModel('Financials');
			...
			if (RealestatenowHelper::checkArray($data))
			{
				...
				RealestatenowHelper::xls($data,'Financials_'.$date->format('jS_F_Y'),'Financials exported ('.$date->format('jS F, Y').')','financials');
			}
		}
		// [Interpretation 8934] Redirect to the list screen with error.
		$message = JText::_('COM_REALESTATENOW_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_realestatenow&view=financials', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('financial.import', 'com_realestatenow') && $user->authorise('core.import', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkObject($headers))
			{
				...
				$session->set('financial_VDM_IMPORTHEADERS', $headers);
				$session->set('backto_VDM_IMPORT', 'financials');
				$session->set('dataType_VDM_IMPORTINTO', 'financial');
				// [Interpretation 8961] Redirect to import view.
				$message = JText::_('COM_REALESTATENOW_IMPORT_SELECT_FILE_FOR_FINANCIALS');
