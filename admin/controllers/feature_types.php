...
/**
 * Feature_types Controller
 */
class RealestatenowControllerFeature_types extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_REALESTATENOW_FEATURE_TYPES';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Feature_type', $prefix = 'RealestatenowModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('feature_type.export', 'com_realestatenow') && $user->authorise('core.export', 'com_realestatenow'))
		{
			...
			$model = $this->getModel('Feature_types');
			...
			if (RealestatenowHelper::checkArray($data))
			{
				...
				RealestatenowHelper::xls($data,'Feature_types_'.$date->format('jS_F_Y'),'Feature types exported ('.$date->format('jS F, Y').')','feature types');
			}
		}
		// [Interpretation 8934] Redirect to the list screen with error.
		$message = JText::_('COM_REALESTATENOW_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_realestatenow&view=feature_types', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('feature_type.import', 'com_realestatenow') && $user->authorise('core.import', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkObject($headers))
			{
				...
				$session->set('feature_type_VDM_IMPORTHEADERS', $headers);
				$session->set('backto_VDM_IMPORT', 'feature_types');
				$session->set('dataType_VDM_IMPORTINTO', 'feature_type');
				// [Interpretation 8961] Redirect to import view.
				$message = JText::_('COM_REALESTATENOW_IMPORT_SELECT_FILE_FOR_FEATURE_TYPES');
