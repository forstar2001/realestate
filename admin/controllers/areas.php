 ...
 * Areas Controller
 */
class RealestatenowControllerAreas extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_REALESTATENOW_AREAS';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Area', $prefix = 'RealestatenowModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('area.export', 'com_realestatenow') && $user->authorise('core.export', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkArray($data))
			{
				...
				RealestatenowHelper::xls($data,'Areas_'.$date->format('jS_F_Y'),'Areas exported ('.$date->format('jS F, Y').')','areas');
			}
		}
		// [Interpretation 8934] Redirect to the list screen with error.
		$message = JText::_('COM_REALESTATENOW_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_realestatenow&view=areas', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('area.import', 'com_realestatenow') && $user->authorise('core.import', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkObject($headers))
			{
				...
				$message = JText::_('COM_REALESTATENOW_IMPORT_SELECT_FILE_FOR_AREAS');
