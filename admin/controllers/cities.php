 ...
 * Cities Controller
 */
class RealestatenowControllerCities extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_REALESTATENOW_CITIES';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'City', $prefix = 'RealestatenowModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('city.export', 'com_realestatenow') && $user->authorise('core.export', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkArray($data))
			{
				...
				RealestatenowHelper::xls($data,'Cities_'.$date->format('jS_F_Y'),'Cities exported ('.$date->format('jS F, Y').')','cities');
			}
		}
		// [Interpretation 8934] Redirect to the list screen with error.
		$message = JText::_('COM_REALESTATENOW_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_realestatenow&view=cities', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('city.import', 'com_realestatenow') && $user->authorise('core.import', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkObject($headers))
			{
				...
				$message = JText::_('COM_REALESTATENOW_IMPORT_SELECT_FILE_FOR_CITIES');
