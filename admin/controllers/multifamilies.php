...
/**
 * Multifamilies Controller
 */
class RealestatenowControllerMultifamilies extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_REALESTATENOW_MULTIFAMILIES';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Multifamily', $prefix = 'RealestatenowModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('multifamily.export', 'com_realestatenow') && $user->authorise('core.export', 'com_realestatenow'))
		{
			...
			$model = $this->getModel('Multifamilies');
			...
			if (RealestatenowHelper::checkArray($data))
			{
				...
				RealestatenowHelper::xls($data,'Multifamilies_'.$date->format('jS_F_Y'),'Multifamilies exported ('.$date->format('jS F, Y').')','multifamilies');
			}
		}
		// [Interpretation 8934] Redirect to the list screen with error.
		$message = JText::_('COM_REALESTATENOW_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_realestatenow&view=multifamilies', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('multifamily.import', 'com_realestatenow') && $user->authorise('core.import', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkObject($headers))
			{
				...
				$session->set('multifamily_VDM_IMPORTHEADERS', $headers);
				$session->set('backto_VDM_IMPORT', 'multifamilies');
				$session->set('dataType_VDM_IMPORTINTO', 'multifamily');
				// [Interpretation 8961] Redirect to import view.
				$message = JText::_('COM_REALESTATENOW_IMPORT_SELECT_FILE_FOR_MULTIFAMILIES');
