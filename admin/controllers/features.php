 ...
 * Features Controller
 */
class RealestatenowControllerFeatures extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_REALESTATENOW_FEATURES';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Feature', $prefix = 'RealestatenowModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('feature.export', 'com_realestatenow') && $user->authorise('core.export', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkArray($data))
			{
				...
				RealestatenowHelper::xls($data,'Features_'.$date->format('jS_F_Y'),'Features exported ('.$date->format('jS F, Y').')','features');
			}
		}
		// [Interpretation 8934] Redirect to the list screen with error.
		$message = JText::_('COM_REALESTATENOW_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_realestatenow&view=features', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('feature.import', 'com_realestatenow') && $user->authorise('core.import', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkObject($headers))
			{
				...
				$session->set('feature_VDM_IMPORTHEADERS', $headers);
				$session->set('backto_VDM_IMPORT', 'features');
				$session->set('dataType_VDM_IMPORTINTO', 'feature');
				// [Interpretation 8961] Redirect to import view.
				$message = JText::_('COM_REALESTATENOW_IMPORT_SELECT_FILE_FOR_FEATURES');
