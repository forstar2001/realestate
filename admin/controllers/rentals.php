 ...
 * Rentals Controller
 */
class RealestatenowControllerRentals extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_REALESTATENOW_RENTALS';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Rental', $prefix = 'RealestatenowModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('rental.export', 'com_realestatenow') && $user->authorise('core.export', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkArray($data))
			{
				...
				RealestatenowHelper::xls($data,'Rentals_'.$date->format('jS_F_Y'),'Rentals exported ('.$date->format('jS F, Y').')','rentals');
			}
		}
		// [Interpretation 8934] Redirect to the list screen with error.
		$message = JText::_('COM_REALESTATENOW_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_realestatenow&view=rentals', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('rental.import', 'com_realestatenow') && $user->authorise('core.import', 'com_realestatenow'))
		{
			...
			if (RealestatenowHelper::checkObject($headers))
			{
				...
				$session->set('rental_VDM_IMPORTHEADERS', $headers);
				$session->set('backto_VDM_IMPORT', 'rentals');
				$session->set('dataType_VDM_IMPORTINTO', 'rental');
				// [Interpretation 8961] Redirect to import view.
				$message = JText::_('COM_REALESTATENOW_IMPORT_SELECT_FILE_FOR_RENTALS');
