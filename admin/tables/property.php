 ...
 */
class RealestatenowTableProperty extends JTable
{
	/**
	...
	{
		parent::__construct('#__realestatenow_property', 'id', $db);

		// [Interpretation 4642] Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_realestatenow.property'));
	}	
	...
	/**
	 * Overload the store method for the Property table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('property', 'RealestatenowTable');

			if ($table->load(array('alias' => $this->alias, 'catid' => $this->catid)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_REALESTATENOW_PROPERTY_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_realestatenow.property.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_realestatenow.property.'.(int) $this->$k;
	}
