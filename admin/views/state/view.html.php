 ...
 * State View class
 */
class RealestatenowViewState extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = RealestatenowHelper::getActions('state', $this->item);
		// get input
		...
		$this->vwaproperty_listings = $this->get('Vwaproperty_listings');
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_REALESTATENOW_STATE_NEW' : 'COM_REALESTATENOW_STATE_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			...
			else
			{
				...
				{
					JToolbarHelper::versions('com_realestatenow.state', $this->item->id);
				}
				if ($this->canDo->get('state.create'))
				{
		...
		$help_url = RealestatenowHelper::getHelpUrl('state');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_REALESTATENOW_STATE_NEW' : 'COM_REALESTATENOW_STATE_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_realestatenow/assets/css/state.css", (RealestatenowHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_realestatenow/views/state/submitbutton.js", (RealestatenowHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 
