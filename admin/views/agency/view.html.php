 ...
 * Agency View class
 */
class RealestatenowViewAgency extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = RealestatenowHelper::getActions('agency', $this->item);
		// get input
		...
		$this->vwgproperty_listings = $this->get('Vwgproperty_listings');
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_REALESTATENOW_AGENCY_NEW' : 'COM_REALESTATENOW_AGENCY_EDIT'), 'pencil-2 article-add');
		// [Interpretation 11906] Built the actions for new and existing records.
		if (RealestatenowHelper::checkString($this->referral))
		{
			if ($this->canDo->get('agency.create') && $isNew)
			{
				// [Interpretation 11918] We can create the record.
				JToolBarHelper::save('agency.save', 'JTOOLBAR_SAVE');
			}
			elseif ($this->canDo->get('agency.edit'))
			{
			...
			if ($isNew)
			{
				...
				JToolBarHelper::cancel('agency.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				// [Interpretation 11940] We can close it.
				JToolBarHelper::cancel('agency.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('agency.create'))
				{
					JToolBarHelper::apply('agency.apply', 'JTOOLBAR_APPLY');
					JToolBarHelper::save('agency.save', 'JTOOLBAR_SAVE');
					JToolBarHelper::custom('agency.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				JToolBarHelper::cancel('agency.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				if ($this->canDo->get('agency.edit'))
				{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('agency.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('agency.edit') && $canVersion)
				{
					JToolbarHelper::versions('com_realestatenow.agency', $this->item->id);
				}
				if ($this->canDo->get('agency.create'))
				{
					JToolBarHelper::custom('agency.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				JToolBarHelper::cancel('agency.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		...
		$help_url = RealestatenowHelper::getHelpUrl('agency');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_REALESTATENOW_AGENCY_NEW' : 'COM_REALESTATENOW_AGENCY_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_realestatenow/assets/css/agency.css", (RealestatenowHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_realestatenow/views/agency/submitbutton.js", (RealestatenowHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 
