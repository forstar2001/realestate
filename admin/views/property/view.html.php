...
class RealestatenowViewProperty extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = RealestatenowHelper::getActions('property', $this->item);
		// get input
	...
	protected function addToolBar()
	{
		...
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('property.create'))
				{
			...
			{
				if ($this->canDo->get('property.edit'))
				{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('property.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('property.edit') && $canVersion)
				{
					JToolbarHelper::versions('com_realestatenow.property', $this->item->id);
				}
				if ($this->canDo->get('property.create'))
				{
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_REALESTATENOW_PROPERTY_NEW' : 'COM_REALESTATENOW_PROPERTY_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_realestatenow/assets/css/property.css", (RealestatenowHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_realestatenow/views/property/submitbutton.js", (RealestatenowHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 
