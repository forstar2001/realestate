...
/**
 * Realestatenow Model for State
 */
class RealestatenowModelState extends JModelList
{
	/**
	 * Model user data.
	 *
	...
	protected function getListQuery()
	{
		...
		{
			$query->where('a.stateid = ' . $db->quote($checkValue));
		}
		elseif (is_numeric($checkValue))
		{
			$query->where('a.stateid = ' . $checkValue);
		}
		else
		{
			return false;
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.state.access', 'com_realestatenow'))
		{
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('COM_REALESTATENOW_NOT_AUTHORISED_TO_VIEW_STATE'), 'error');
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.state.propdesc', &$_propdesc, &$params, 0));
				...
				$item->idPropidImageH = $this->getIdPropidImageAeaf_H($item->id);
			}
		}
	 ...
	 *
	 */
	public function getIdPropidImageAeaf_H($id)
	{
		...
		if ($db->getNumRows())
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.state.description', &$_description, &$params, 0));
