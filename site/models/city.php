...
/**
 * Realestatenow Model for City
 */
class RealestatenowModelCity extends JModelList
{
	/**
	 * Model user data.
	 *
	...
	protected function getListQuery()
	{
		...
		{
			$query->where('a.cityid = ' . $db->quote($checkValue));
		}
		elseif (is_numeric($checkValue))
		{
			$query->where('a.cityid = ' . $checkValue);
		}
		else
		{
			return false;
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.city.access', 'com_realestatenow'))
		{
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('COM_REALESTATENOW_NOT_AUTHORISED_TO_VIEW_CITY'), 'error');
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.city.propdesc', &$_propdesc, &$params, 0));
				...
				$item->idPropidImageH = $this->getIdPropidImageCaea_H($item->id);
			}
		}
	 ...
	 *
	 */
	public function getIdPropidImageCaea_H($id)
	{
		...
		if ($db->getNumRows())
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.city.description', &$_description, &$params, 0));
	...
	public function getCity()
	{
		...
		$query->select($db->quoteName(
			array('a.id','a.asset_id','a.image','a.description','a.name','a.stateid','a.alias','a.owncoords','a.latitude','a.longitude','a.published','a.created_by','a.modified_by','a.created','a.modified','a.version','a.hits','a.ordering'),
			array('id','asset_id','image','description','name','stateid','alias','owncoords','latitude','longitude','published','created_by','modified_by','created','modified','version','hits','ordering')));
