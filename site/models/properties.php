...
// import the Joomla modellist library
    ...
    /**
     * Realestatenow Model for Properties
     */
    class RealestatenowModelProperties extends ListModel
    {
        ...
        protected function populateState($ordering = null, $direction = null)
        {
            ...
            $list = $app->getUserStateFromRequest(
                ...
                'list',
                [
                    'limit'=>$this->params->get('listlimit', 5),
                    'page'=>1,
                    'sort'=>$this->params->get('listsort','name_asc')
                ],
                'array'
            );
            ...
            $default_filters =  array_fill_keys ($this->filter_fields, '');
        
            $filter = $app->getUserStateFromRequest ($this->context . '.filter', 'filter',$default_filters , 'array');
    
            $filter = array_merge($default_filters,$filter);
            
            if( $transactiontype = $app->input->get('transactiontype',false,'INT') )
        ...
        protected function getListQuery()
        {
            ...
            $limit = $this->getState('list', [ 'limit'=>5 ]  )['limit'];
        ...
        {
        $query = clone $query;
        $query->clear('select' )->clear('limit' )->clear('offset' )->select('a.id' );
        ...
        public function getItems()
        {
            ...
            if (!$user->authorise('site.properties.access', 'com_realestatenow'))
            {
                $app = JFactory::getApplication();
                
                $app->enqueueMessage(JText::_('COM_REALESTATENOW_NOT_AUTHORISED_TO_VIEW_PROPERTIES'), 'error');
            ...
            if (RealestatenowHelper::checkArray($items)) {
                ...
                foreach ($items as &$item) {
                    ...
                    $this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.properties.propdesc', &$_propdesc, &$this->params, 0));
                    ...
                    $this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.category.agent_bio', &$_agent_bio, &$this->params, 0));
                    
                    // Checking if agent_bio has uikit components that must be loaded.
                    $this->uikitComp = RealestatenowHelper::getUikitComp($item->agent_bio,$this->uikitComp);
                    
                    // set the global property_id value.
                    $this->a_property_id = $item->id;
                    
                    // [Interpretation 1890] set idPropidImageJ to the $item object.
                    $item->idPropidImageJ = $this->getIdPropidImageCeef_J($item);
                    
                    // [Interpretation 1890] set catidIdCategoriesB to the $item object.
                    $item->catidIdCategoriesB = $this->getIdCatidPropertyCeeb_B($item->catid);
                }
            }
         ...
         *
         */
        public function getIdPropidImageCeef_J($item)
        {
            ...
            $events = [[
                    ...
                    'params'=>['com_realestatenow.properties.description', &$_description, &$this->params, 0]
            ]];
            
            return RealestatenowImageHelper::getGetPropertyImagesByPropertyId($item->id, $events);
            ...
            if ($db->getNumRows())
            {
                ...
                foreach ($items as $nr => &$item)
                {
                    ...
                    $this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.properties.description', &$_description, &$this->params, 0));
         ...
         *
         */
        public function getIdCatidPropertyCeeb_B($catid)
        {
            // Get a db connection.
            $db = JFactory::getDbo();
        ...
        public function getStateList()
        {
            ...
            // Get from #__realestatenow_state as a
            $query->select($db->quoteName(
        ...
        public function getCategory()
        {
            ...
            */
            ...
            // [Interpretation 1890] set idCatidPropertyB to the $data object.
            $data->idCatidPropertyB = $this->getIdCatidPropertyCeeb_B($data->id);
