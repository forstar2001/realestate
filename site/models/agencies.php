...
/**
 * Realestatenow Model for Agencies
 */
class RealestatenowModelAgencies extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		// [Interpretation 1592] Get from #__realestatenow_country as b
		$query->select($db->quoteName(
			array('b.name'),
			array('country_name')));
		$query->join('LEFT', ($db->quoteName('#__realestatenow_country', 'b')) . ' ON (' . $db->quoteName('a.countryid') . ' = ' . $db->quoteName('b.id') . ')');

		// [Interpretation 1592] Get from #__realestatenow_state as c
		$query->select($db->quoteName(
			array('c.name'),
			array('state_name')));
		$query->join('LEFT', ($db->quoteName('#__realestatenow_state', 'c')) . ' ON (' . $db->quoteName('a.stateid') . ' = ' . $db->quoteName('c.id') . ')');

		// [Interpretation 1592] Get from #__realestatenow_city as d
		$query->select($db->quoteName(
			array('d.name'),
			array('city_name')));
		$query->join('LEFT', ($db->quoteName('#__realestatenow_city', 'd')) . ' ON (' . $db->quoteName('a.cityid') . ' = ' . $db->quoteName('d.id') . ')');
		$query->where('a.access IN (' . implode(',', $this->levels) . ')');
		// [Interpretation 2167] Get where a.published is 1
		$query->where('a.published = 1');
		// [Interpretation 2167] Get where a.access is 1
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.agencies.access', 'com_realestatenow'))
		{
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('COM_REALESTATENOW_NOT_AUTHORISED_TO_VIEW_AGENCIES'), 'error');
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.agencies.description', &$_description, &$params, 0));
