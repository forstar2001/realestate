...
/**
 * Realestatenow Model for States
 */
class RealestatenowModelStates extends JModelList
{
	/**
	 * Model user data.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select($db->quoteName(
			array('a.id','a.asset_id','a.name','a.alias','a.countryid','a.image','a.description','a.owncoords','a.latitude','a.longitude','a.published','a.created_by','a.modified_by','a.created','a.modified','a.version','a.hits','a.ordering'),
			array('id','asset_id','name','alias','countryid','image','description','owncoords','latitude','longitude','published','created_by','modified_by','created','modified','version','hits','ordering')));
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.states.access', 'com_realestatenow'))
		{
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('COM_REALESTATENOW_NOT_AUTHORISED_TO_VIEW_STATES'), 'error');
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.states.description', &$_description, &$params, 0));
