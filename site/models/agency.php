...
/**
 * Realestatenow Model for Agency
 */
class RealestatenowModelAgency extends JModelList
{
	/**
	 * Model user data.
	 *
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.agency.access', 'com_realestatenow'))
		{
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('COM_REALESTATENOW_NOT_AUTHORISED_TO_VIEW_AGENCY'), 'error');
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.agency.propdesc', &$_propdesc, &$params, 0));
				...
				$item->idPropidImageH = $this->getIdPropidImageCaba_H($item->id);
			}
		}
	 ...
	 *
	 */
	public function getIdPropidImageCaba_H($id)
	{
		...
		$query->select($db->quoteName(
			array('h.path','h.filename','h.type','h.remote','h.title'),
			array('path','filename','type','remote','title')));
	...
	$this->_dispatcher = JEventDispatcher::getInstance();
		...
		$this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.agency.description', &$_description, &$params, 0));
