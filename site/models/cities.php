...
/**
 * Realestatenow Model for Cities
 */
class RealestatenowModelCities extends JModelList
{
	/**
	 * Model user data.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select($db->quoteName(
			array('a.id','a.asset_id','a.name','a.stateid','a.alias','a.description','a.image','a.owncoords','a.latitude','a.longitude','a.published','a.created_by','a.modified_by','a.created','a.modified','a.version','a.hits','a.ordering'),
			array('id','asset_id','name','stateid','alias','description','image','owncoords','latitude','longitude','published','created_by','modified_by','created','modified','version','hits','ordering')));
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.cities.access', 'com_realestatenow'))
		{
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('COM_REALESTATENOW_NOT_AUTHORISED_TO_VIEW_CITIES'), 'error');
		...
		if (RealestatenowHelper::checkArray($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->_dispatcher->trigger("onContentPrepare", array('com_realestatenow.cities.description', &$_description, &$params, 0));
				...
				// [Interpretation 1917] set idCityidPropertyC to the $item object.
				$item->idCityidPropertyC = $this->getIdCityidPropertyCccb_C($item->id);
			}
		}
	 ...
	 *
	 */
	public function getIdCityidPropertyCccb_C($id)
	{
		...
		// [Interpretation 2747] Get from #__realestatenow_property as c
		$query->select($db->quoteName(
			array('c.id'),
			array('id')));
		$query->from($db->quoteName('#__realestatenow_property', 'c'));
		$query->where('c.cityid = ' . $db->quote($id));
		$query->where('c.access IN (' . implode(',', $this->levels) . ')');
		// [Interpretation 2167] Get where c.published is 1
