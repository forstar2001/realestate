...
/-------------------------------------------------------------------------------------------------------/
	...
	@subpackage		transtypepropertylistings.php
...
?>
<!-- Transaction Type Listings Layout -- Updated 2018-09-15 -->
...
<?php if (JComponentHelper::getParams('com_realestatenow')->get('properties_thumb_type') == 1) : ?>
    ...
    <?php if( is_array($displayData->idPropidImageJ) && ( count($displayData->idPropidImageJ) > 0 ) ): ?>
        <?php foreach ($displayData->idPropidImageJ as $idPropidImageJ): ?>
                <li>
                    <a href="<?php echo 'index.php?option=com_realestatenow&view=property&shownav=1&offset='.$displayData->offset.'&id='.$displayData->id;?>" title="<?php echo $displayData->name;?>" rel="">
...
<?php endif; ?>
    ...
    <?php else: ?>
            <div>
                <a href="<?php echo 'index.php?option=com_realestatenow&view=property&shownav=1&offset='.$displayData->offset.'&id='.$displayData->id;?>"
                   title="<?php echo $displayData->name;?>"
                   rel="">
                    <img class="uk-thumbnail uk-thumbnail-medium"
                         src="<?php echo JURI::root().'/media/com_realestatenow/images/No_image_available.png'; ?>"
                    >
...
<?php if (JComponentHelper::getParams('com_realestatenow')->get('adline') == 1) : ?>
        <a href="<?php echo 'index.php?option=com_realestatenow&view=property&shownav=1&offset='.$displayData->offset.'&id='.$displayData->id;?>" title="<?php echo $displayData->name;?>" rel="">
            <<?php echo (JComponentHelper::getParams('com_realestatenow')->get('linksize')); ?> style="color:<?php echo (JComponentHelper::getParams('com_realestatenow')->get('linkcolor')); ?>; font-weight:<?php echo (JComponentHelper::getParams('com_realestatenow')->get('linkweight')); ?>;"><?php echo $displayData->name; ?></<?php echo (JComponentHelper::getParams('com_realestatenow')->get('linksize')); ?>>
        </a>
<?php endif; ?>
        <a href="<?php echo 'index.php?option=com_realestatenow&view=property&shownav=1&offset='.$displayData->offset.'&id='.$displayData->id;?>" title="<?php echo $displayData->name;?>" rel="">
            <<?php echo (JComponentHelper::getParams('com_realestatenow')->get('linksize')); ?> class="uk-clearfix uk-text-nowrap">
                <?php echo $displayData->street; ?>
                <?php echo $displayData->city_name.', '.$displayData->state_name.' '.$displayData->postcode; ?>
            </<?php echo (JComponentHelper::getParams('com_realestatenow')->get('linksize')); ?>>
        </a>
        <div>
