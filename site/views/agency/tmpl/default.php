        ...
        <div class="uk-grid uk-flex-middle" uk-grid>
            <div class="uk-width-expand">
                <?php if (empty($this->agency->image)) { ?>
                    <img class="uk-border-circle uk-align-left uk-margin-remove-adjacent" width="200" height="200" src="<?php echo JURI::root() . '/media/com_realestatenow/images/No_image_available.png'; ?>">
                <?php } else { ?>
                    <img class="uk-border-circle" width="100" height="100" src="<?php echo $this->agency->image; ?>">
                <?php } ?>
                <h3 class="uk-card-title uk-margin-remove-bottom"><?php echo $this->agency->name; ?></h3>
                <?php if (!empty($this->agency->phone)) { ?>
                    <p class="uk-text-meta uk-margin-remove-top"><i class="uk-icon-justify uk-icon-phone"></i> <?php echo JText::_('COM_REALESTATENOW_PHONE'); ?>
                        <span><?php echo ' ' . $this->agency->phone; ?></span></p>
                <?php }
                if (!empty($this->agency->fax)) { ?>
                    <p class="uk-text-meta uk-margin-remove-top"><i class="uk-icon-justify uk-icon-fax"></i> <?php echo JText::_('COM_REALESTATENOW_FAX'); ?>
                        <span><?php echo ' ' . $this->agency->fax; ?></span></p>
                <?php }
                if (!empty($this->agency->license)) { ?>
                    <p class="uk-text-meta uk-margin-remove-top"><i class="uk-icon-justify fa fa-id-card-o"></i> <?php echo JText::_('COM_REALESTATENOW_LICENSE'); ?>
                        <span><?php echo ' ' . $this->agency->license; ?></span></p>
                <?php } ?>
            </div>
        </div>
    </div>
    <div class="uk-card-body">
        ...
        <?php else: ?>
            <div class="uk-alert uk-alert-warning" data-uk-alert><a href="" class="uk-alert-close uk-close"></a>
                <p><?php echo JText::_('COM_REALESTATENOW_NO_PROPERTIES_WERE_FOUND_FOR_THIS_AGENCY'); ?></p>
            </div>
        <?php endif; ?>
    </div>
</div>
    ...
    <script>
        ...
        function initMap() {
            ...
            map = new Microsoft.Maps.Map(document.getElementById("map"), {
                ...
                zoom: <?php if ($this->params->get('agency_map_zoom') != '') echo $this->params->get('agency_map_zoom'); else {
                    echo '10';
                }?>
            });
