...
/**
 * Realestatenow View class for the State
 */
class RealestatenowViewState extends JViewLegacy
{
	...
	{		
		...
		$this->liststate = $this->get('ListState');
		// add a hit to the state
		if ($this->hit($this->liststate->id))
		{
			$this->liststate->hits++;
		}
	 ...
	 /**
	 * Increment the hit counter for the state.
	 *
	 * @param   integer  $pk  Primary key of the state to increment.
	 *
	...
	public function hit($pk = 0)
	{
		if ($pk)
		{
			...
			$query->update($db->quoteName('#__realestatenow_state'))->set($fields)->where($conditions);
	...
	protected function _prepareDocument()
	{
		...
		$this->document->addStyleSheet(JURI::root(true) .'/components/com_realestatenow/assets/css/state.css', (RealestatenowHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
