            ...
            if (is_array ($this->items) && (count ($this->items) > 0)):
                ...
                foreach($this->items as $key=>&$item):
                    ...
                    if($this->params->get('properties_display') == 2):
                        $this->response->listhtml[] = JLayoutHelper::render('openhousespanellistings',$item);
                        if( !empty($item->latitude) && !empty($item->longitude) ):
                            $this->response->mapOptions->infoWindowContent[] = [JLayoutHelper::render ('openhousesinfobox', $item)];
                        endif;
                    elseif($this->params->get('properties_display') == 3):
                        $this->response->listhtml[] = JLayoutHelper::render('openhouseslandingpage',$item);
                        if( !empty($item->latitude) && !empty($item->longitude) ):
                            $this->response->mapOptions->infoWindowContent[] = [JLayoutHelper::render ('openhousesinfobox', $item)];
                        endif;
                    else:
                        $this->response->listhtml[] = JLayoutHelper::render('openhouseslistings',$item);
                        if( !empty($item->latitude) && !empty($item->longitude) ):
                            $this->response->mapOptions->infoWindowContent[] = [JLayoutHelper::render ('openhousesinfobox', $item)];
                        endif;
                    endif;
