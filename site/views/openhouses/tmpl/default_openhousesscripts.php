...
/-------------------------------------------------------------------------------------------------------/
	...
	@subpackage		default_openhousesscripts.php
    ...
    function SearchPropertiesFilter(page,clear)
    {
        ...
        if(clear == 'clear') {
            jQuery.ajax({
                ...
                url: "<?php echo JUri::base();?>index.php?option=com_realestatenow&task=openhouses.display&format=json",
    ...
    function initPagination(options = false) {
        ...
        if (options) {
            ...
            paginationOptions = {
                ...
                totalPages: <?php echo ( $this->active_state_vars['list']['limit'] > 0 )
                    ? ( intval( ceil($this->totalproperties / $this->active_state_vars['list']['limit'] )) )
                    : 1;?>
                ,
                limit: <?php echo
                    !empty( $this->active_state_vars['list']['limit'] )
                        ? $this->active_state_vars['list']['limit']
                        : 1;?>,
