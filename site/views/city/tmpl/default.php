...
<?php echo $this->toolbar->render(); ?><!-- Single city view start -->

<div class="uk-container">
    <div class="uk-block">
        <div>
            <h3 class="uk-article-title"><?php echo $this->city->name; ?></h3>
        </div>
    </div>
    <?php if ($this->params->get('show_city_image') == 1) : ?>
        <div class="uk-grid">
            <div class="uk-width-1-3">
                <?php if (empty($this->city->image)) { ?>
                    <div><img class='uk-thumbnail'
                              src="<?php echo JURI::root() . '/media/com_realestatenow/images/No_image_available.png'; ?>">
                    </div>
                <?php } else { ?>
                    <div><img class='uk-thumbnail' src="<?php echo $this->city->image; ?>"
                              alt="<?php echo $this->city->name; ?>"></div>
                <?php } ?>
            </div>
            <div></div>
        </div>
    <?php endif; ?>
    <?php if ($this->items): ?>
        <?php if ($this->params->get('city_properties_display') == 2) : ?>
            <?php echo $this->loadTemplate('city-properties-tab'); ?>
        <?php elseif ($this->params->get('city_properties_display') == 3) : ?>
            <?php echo $this->loadTemplate('city-properties-accordion'); ?>
        <?php else: ?>
            <?php echo $this->loadTemplate('city-properties-grid'); ?>
        <?php endif; ?>
    <?php else: ?>
        <div class="uk-alert uk-alert-warning" data-uk-alert><a href="" class="uk-alert-close uk-close"></a>
            <p><?php echo JText::_('COM_REALESTATENOW_NO_CITIES_WERE_FOUND'); ?></p>
        </div>
    <?php endif; ?>
...
<?php if ($this->params->get('map_provider') == '1') { ?>
    <script>
        ...
        function initMap() {
            ...
            var place = {
                lat: <?php echo ($this->city->latitude != '') ? $this->city->latitude : '47.6149942';?>,
                lng: <?php echo ($this->city->longitude != '') ? $this->city->longitude : '-122.4759886'; ?>
            };

            var zoom = <?php echo ($this->params->get('city_map_zoom') != '') ? $this->params->get('city_map_zoom') : '10';?>;
...
if ($this->params->get('map_provider') == '2') {
    ...
    <script>
        ...
        function initMap() {
            ...
            var lat = <?php echo ($this->city->latitude != '') ? $this->city->latitude : '47.6149942';?>;
            var long = <?php echo ($this->city->longitude != '') ? $this->city->longitude : '-122.4759886'; ?>;
            ...
            map = new Microsoft.Maps.Map(document.getElementById("map"), {
                ...
                zoom: <?php if ($this->params->get('city_map_zoom') != '') echo $this->params->get('city_map_zoom'); else {
                    echo '10';
                }?>
            });
